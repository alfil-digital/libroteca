<x-app-layout> <!-- Utiliza el componente lateral de diseño de la aplicación -->
    <x-slot name="header"> <!-- Define el contenido para la sección de cabecera -->
        <h2 class="h4 mb-0 text-dark">
            {{ __('Gestión de Roles') }} <!-- Título principal de la página -->
        </h2>
    </x-slot>
    ...
    <div class="card shadow-sm border-0 mt-4"> <!-- Tarjeta con sombra suave y sin bordes -->
        <div class="card-header bg-white py-3 d-flex justify-content-between align-items-center">
            ...
            <h5 class="card-title mb-0">Listado de Roles</h5> <!-- Título del listado -->
            <a href="{{ route('roles.create') }}" class="btn btn-primary btn-sm rounded-pill px-4">
                <!-- Botón primario redondeado -->
                <i class="bi bi-plus-lg me-1"></i> {{ __('Agregar Nuevo Rol') }} <!-- Texto del botón -->
            </a>
        </div>
        ...
        <div class="card-body p-0"> <!-- Cuerpo de la tarjeta sin padding para que la tabla sea a sangre -->
            ...
            <div class="table-responsive"> <!-- Envuelve la tabla para que sea responsiva en móviles -->
                <table class="table table-hover align-middle mb-0">
                    ...
                    <tbody>
                        @foreach($roles as $role) <!-- Recorre cada usuario de la base de datos -->
                            <tr>
                                <td>{{ $role->id }}</td>
                                <td class="ps-4 fw-medium text-dark">{{ $role->name }}</td>
        ...
        @if($roles->hasPages()) <!-- Solo muestra la paginación si hay más de una página -->
